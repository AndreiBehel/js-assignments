...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    ...
    var vectMagnMult = Math.sqrt(Math.pow(x1, 2) + Math.pow(y1, 2)) * Math.sqrt(Math.pow(x2, 2) + Math.pow(y2, 2));
    return Math.acos( dotProduct / vectMagnMult);
}
...
function getLastDigit(value) {
    var numRepr = value.toString();
    return +numRepr[numRepr.length - 1];
}
...
function toNumber(value, def) {
    ...
    return convValue ? convValue : def;
}
